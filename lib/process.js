...
function isDockerEnabled(type) {
  switch (type) {
    case CLI:
    case JAVAC:
      return atom.config.get('open-diabetes-vault-suite.cli.executable') === MODE_DOCKER;
    case PLOT:
      return atom.config.get('open-diabetes-vault-suite.plot.executable') === MODE_DOCKER;
  }
}
...
function getDocker() {
  ...
  return new Promise((resolve, reject) => {
    if (atom.config.get('open-diabetes-vault-suite.docker.remote')) {
      // load tls files asynchronously
      const cert = fs.readFile(atom.config.get('open-diabetes-vault-suite.docker.cert'));
      const key = fs.readFile(atom.config.get('open-diabetes-vault-suite.docker.key'));
      let cacert;
      if (atom.config.get('open-diabetes-vault-suite.docker.noca')) {
        process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
        cacert = Promise.resolve(undefined);
      } else cacert = fs.readFile(atom.config.get('open-diabetes-vault-suite.docker.cacert'));

      Promise.all([cert, key, cacert])
      // after all tls files are loaded
        .then(files => {
          const docker = new Docker({
            protocol: atom.config.get('open-diabetes-vault-suite.docker.protocol'),
            host: atom.config.get('open-diabetes-vault-suite.docker.host'),
            port: atom.config.get('open-diabetes-vault-suite.docker.port'),
...
function getCli() {
  if (atom.config.get('open-diabetes-vault-suite.cli.executable') === 'native')
    return path.resolve(getModulePath(), '..', 'bin', 'odv', 'OpenDiabetesVault.jar');
  else return atom.config.get('open-diabetes-vault-suite.cli.custom');
}
    ...
    .catch(stderr => {
      const notification = atom.notifications.addWarning('Invalid CLI defined', {
        ...
        buttons: [{
          ...
          text: 'Open Settings',
          onDidClick: () => {
            atom.workspace.open('atom://config/packages/open-diabetes-vault-suite');
...
function getPlot() {
  if (atom.config.get('open-diabetes-vault-suite.plot.executable') === 'native')
    return path.resolve(getModulePath(), '..', 'bin', 'plotteria', 'plot.py');
  else return atom.config.get('open-diabetes-vault-suite.plot.custom');
}

function getPlotConfig() {
  if (atom.config.get('open-diabetes-vault-suite.plot.config-native'))
    return path.resolve(getModulePath(), '..', 'bin', 'plotteria', 'config.ini');
  return atom.config.get('open-diabetes-vault-suite.plot.config');
}
...
class Process {
   ...
   *
   * @param {string} command Command defined in settings. Assumes `open-diabetes-vault-suite.commands` namespace.
  ...
  constructor(command, args = {}, type = CLI) {
    switch (type) {
      case CLI:
        if (atom.config.get('open-diabetes-vault-suite.cli.executable') === MODE_DOCKER) {
          this.docker = true;
          ...
          this.docker = false;
          args.java = atom.config.get('open-diabetes-vault-suite.paths.java');
          args.cli = getCli();
        }
        break;
      case JAVAC:
        if (atom.config.get('open-diabetes-vault-suite.cli.executable') === MODE_DOCKER) {
          ...
          this.docker = false;
          args.javac = atom.config.get('open-diabetes-vault-suite.paths.javac');
          args.cli = getCli();
        }
        break;
      case PLOT:
        if (atom.config.get('open-diabetes-vault-suite.plot.executable') === MODE_DOCKER) {
          this.docker = true;
          ...
          this.docker = false;
          args.python = atom.config.get('open-diabetes-vault-suite.paths.python');
          args.plot = getPlot();
          ...
          getDocker().then(docker => {
            docker.run(this.docker_image, this.args,
              [new StdoutWritable(this), new StderrWritable(this)],
              {
                ...
                HostConfig: {
                  Binds: this.binds,
                  AutoRemove: atom.config.get('open-diabetes-vault-suite.docker.cleanup')
                },
...
function buildCommand(command, arguments = {}) {
  let cmd = atom.config.get('open-diabetes-vault-suite.commands.' + command);
